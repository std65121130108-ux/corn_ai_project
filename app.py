                ...
                if st.button("🚀 วินิจฉัยรูปนี้"):
                    if model:
                        with st.spinner("AI กำลังทำงาน..."):
                            ...
                            # --- [จุดที่แก้ไข] แสดงผลลัพธ์ทันที ---
                            st.success("✅ บันทึกผลสำเร็จ!")
                            
                            # สร้างกล่องแสดงผลชั่วคราวเพื่อให้เห็นค่า
                            res_color = "green" if "ปกติ" in final_res else "red"
                            st.markdown(f"""
                                <div style="padding:15px; background:#f1f3f4; border-radius:10px; border-left: 5px solid {res_color}; margin-top:10px;">
                                    <h4 style="margin:0; color:#333;">ผล: {final_res}</h4>
                                    <p style="margin:0; color:#666;">ความมั่นใจ: {conf:.2f}%</p>
                                </div>
                            """, unsafe_allow_html=True)
                            
                            time.sleep(2.5) # หน่วงเวลาเพิ่มขึ้น เพื่อให้อ่านผลทัน
                            force_rerun()
                    else: 
                ...
                if "ยังไม่ตรวจ" in filter_option:
                    ...
                    if st.button(f"⚡ วิเคราะห์ทั้งหมด ({len(image_list)} เคส)"):
                        if model:
                            ...
                            status_txt.success("✅ เสร็จสิ้นทั้งหมด!")
                            time.sleep(1.5)
                            force_rerun()
