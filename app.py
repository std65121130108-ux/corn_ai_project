...
import gdown
import urllib.parse
import requests # จำเป็นต้องใช้สำหรับดึงรูปจาก URL
...
# --- 2. CSS ตกแต่ง (คงธีมเดิมที่คุณต้องการ) ---
def local_css():
    st.markdown("""
    <style>
...
def get_image_list(filter_mode):
    try:
        ...
        # Query ใหม่: Join ตาราง plant_cases กับ media_files
        base_sql = """
...
with c2:
    filter_option = st.radio(
        ...
        index=0 # Default เป็นทั้งหมด เพื่อให้เห็นภาพรวมก่อน
    )
...
if len(image_list) > 0:
    ...
    # --- ปุ่มนำทาง (ดีไซน์ตามที่คุณต้องการ) ---
